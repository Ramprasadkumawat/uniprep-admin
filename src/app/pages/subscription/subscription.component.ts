...
export class SubscriptionComponent implements OnInit {
    ...
    getSubscriptionList(data: any){
        this.subscriptionService.getUserSubscriptionsForAdmin(data).subscribe((response) => {
            ...
            const subscriptions = payload?.subscriptions || payload?.subscription_list || [];
            this.subscriptionList = subscriptions.map((sub: any) => ({
                id: sub?.subscriptionId ?? sub?.id,
                subscription_plan_id: sub?.subscriptionId ?? sub?.subscription_plan_id,
                subscription_type: sub?.userType ?? sub?.subscription_type,
                subscription_plan: sub?.plan_name ?? sub?.subscription_plan,
                plan_name: sub?.plan_name ?? sub?.subscription_plan,
                subtypeName: sub?.userType ?? sub?.subscription_type,
                actualprice: sub?.actual_price ?? sub?.actualprice,
                actual_price: sub?.actual_price ?? sub?.actualprice,
                discountpercentage: sub?.save ?? sub?.discountpercentage,
                save: sub?.save ?? sub?.discountpercentage,
                givenprice: sub?.price ?? sub?.givenprice,
                given_price: sub?.price ?? sub?.givenprice,
                price: sub?.price ?? sub?.givenprice,
                validity: sub?.validity,
                currency: sub?.currency,
                country: sub?.country,
                payment_gateway: sub?.payment_gateway,
                subscriptionType: sub?.subscriptionType ?? sub?.subscription_type,
                userType: sub?.userType,
            }));
            this.subscriptionCount = payload?.count ?? payload?.total_records
            ?? this.subscriptionList.length;
    ...
    submitForm(){
        ...
        const payload = {
            ...formData,
            user_type: formData.subscriptionType,
            subscription_type: formData.subscriptionPlan,
            actual_price: formData.actualPrice,
            given_price: formData.givenPrice,
            save: formData.discountPercentage,
        };
        this.subscriptionService.addSubscriptionPlan(payload).subscribe((response) => {
    ...
    editFormSubmit(){
        ...
        this.subscriptionService.editSubscriptionPlan(payload).subscribe((response) => {
            this.toast.add({severity: 'success',summary: 'Success',detail: response.message});
    ...
    pageChange(event: any): void {
        ...
        this.page = page;
        const params = this.buildSubscriptionParams(page);
    ...
    editoption(data: SubscriptionPlan){
        ...
        console.log('type:', (data as any).userType);
        const resolvedUserType = this.resolveOptionValue(
            this.addUserTypeOptions,
            [(data as any).userType,(data as any).user_type]
        );
        const resolvedSubscriptionType = this.resolveOptionValue(
            this.addSubscriptionTypeOptions,
            [(data as any).subscriptionType,(data as any).subscription_type]
        );
        const resolvedSubscriptionPlan = this.resolveOptionValue(
            this.addSubscriptionTypeOptions,
            [(data as any).plan_name,data.subscription_plan]
        );
        const resolvedCurrency = this.resolveOptionValue(
            this.addCurrencyOptions,
            [data.currency]
        );
        const resolvedCountry = this.resolveOptionValue(
            this.countryOptions,
            [(data as any).country_id,data.country,(data as any).country_name]
        );
        this.editform.patchValue({
            subscriptionType: resolvedUserType,
            subscriptionPlan: resolvedSubscriptionType ?? resolvedSubscriptionPlan,
            country: resolvedCountry,
            actualPrice: (data as any).actual_price ?? data.actualprice,
            validity: data.validity,
            currency: resolvedCurrency,
            discountPercentage: (data as any).save ?? data.discountpercentage,
            givenPrice: (data as any).given_price ?? data.givenprice,
        });
    }
    private resolveOptionValue(options: any[], candidates: any[]): any {
        ...
        const match = options.find((option: any) => {
            ...
            return normalizedCandidates.some(candidate => optionValues.includes(candidate));
        });
        ...
        return match?.value ?? match?.id ?? candidates?.[0] ?? null;
    }
