...
import { AddStoryService } from './addstories.service';
...
@Component({
    ...
    styleUrls: ['./addstories.component.scss'],
    imports: [
...
export class AddStoriesComponent implements OnInit {
  ...
  tutoriallist: any[] = [];
  btntxt = "Submit";
  namentxt="Add Success Stories"
  activeIndex=1;
  perPage:number = 12;
  totalcount: number;
  pageno:number = 1;
  constructor(private userFacade: UserFacadeService, private router: Router, private fb: FormBuilder,
    private http: HttpClient, private toastr: MessageService,private cdr: ChangeDetectorRef , private service:AddStoryService,
...
submitForm() {
  ...
  if (this.form.valid) {
    ...
    if (this.btntxt == "Submit") {
      this.service.addstories(data).subscribe((res) => {
        if (res) {
...
edittutorialdetails(eve:any){
  ...
  this.activeIndex = 0;
  this.btntxt = "Update"; 
  this.namentxt="Update Success Stories"  
