...
export class SubscriberseditprofileComponent implements OnInit {
  ...
  // userReadProgression: any;
  // showSubmodReadProgres: boolean = true;
  // submoduleReadProgression: any;
  // moduleWiseTotalQuesCount: number = 0;
  // moduleWiseReadQuesCount: number = 0;
  ...
  ngOnInit(): void {
    ...
    let subscriberQuizObj = {
      page: "1",
      perpage: "20",
      userid: id,
    };
    // this.getSubscribersQuizProgressList(subscriberQuizObj);
    this.getSubscribersTransactionList(subscriberQuizObj);
    this.getUsageTrackingList()
    // this.getReadProgressionList(subscriberQuizObj);
  ...
  getSubscribers() {
    ...
    this.subscriberService
      .getEditSubscribers(this.subscriberObj)
      .subscribe((response: any) => {
        ...
        if (this.users.length > 0) {
          ...
          this.editform.patchValue({
            ...
            //whatsapp_phoneno: this.users[0]?.whatsapp_phone    
          });
          this.checkWhatsUpOrPhone = this.users[0].whatsapp_phone ? 1 : 0;
          //this.preferredCountrywhatsapp=this.users[0]?.whatsapp_dial_country_code
          ...
          setTimeout(() => {
            this.editform.patchValue({
              source_name: this.users[0]?.source,
            })
          }, 300);
          ...
          if (this.users[0].subscription_id) {
            this.editform.controls["subscription_id"].setValidators(
              Validators.required
            );
            this.editform.controls["subscription_id"].updateValueAndValidity();
            this.editform.controls["credit_plans"].setValidators(
              Validators.required
            );
  ...
  loadSubscriptions() {
    this.subscriptionService.getSubscriptionDropdownList({ studenttype: this.users[0].student_type_id }).subscribe((response) => {
      ...
      //this.editform.get("subscription_id").setValue(subs.id);
    });
  }
  // getUserTypeList() {
  ...
  submitForm() {
    if (this.EditOrUpdate == 'Edit') {
      ...
      //if (this.currentUserId !== 21) {
      //  this.editform.removeControl('subscription_id');
      //}
      // if whats up number is there we need to save whatsapp parameter otherwise phone parameter
      ...
      /*
      if(this.editform?.value?.whatsapp_phoneno?.number == this.users[0].whatsapp_phone){
      }else{
        data.whatsapp_phone=this.editform?.value?.whatsapp_phoneno?.number
      }
      */
  ...
  // getReadProgressionList(data: object) {
  //   this.subscriberService.getReadProgression(data).subscribe((response) => {
  //     console.log(response.data);
  //     if (response.data) {
  //       this.readingProgressioncount = response.totalrecords;
  //       this.preApplicationData = response.data["Pre Admission"];
  //       this.preApplicationKeys = Object.keys(this.preApplicationData);
  //       this.postApplicationData = response.data["Travel And Tourism"];
  //       this.postApplicationKeys = Object.keys(this.postApplicationData);
  //       this.postAdmissionData = response.data["Post Admission"];
  //       this.postAdmissionKeys = Object.keys(this.postAdmissionData);
  //       this.universityData = response.data["University"];
  //       this.universityKeys = Object.keys(this.universityData);
  //       this.careerHubData = response.data["Career Hub"];
  //       this.careerHubKeys = Object.keys(this.careerHubData);
  //       this.lifeAtData = response.data["Life at "];
  //       this.lifeAtKeys = Object.keys(this.lifeAtData);
  //     }
  //   });
  // }
  ...
  //   this.subscriberService.getReadProgression(data).subscribe((response) => {
  //     if (response.data) {
  //       this.userReadProgression = response.data;
  //     }
  //   });
  // }
  // openSubmodReadProgress(moduleId: number, moduleName: string) {
  //   this.showSubmodReadProgres = false;
  //   const currentURL = this.router.url;
  //   const urlParts = currentURL.split("/");
  //   let CurrentUserId: any = urlParts.length > 0 ? urlParts[urlParts.length - 1] : null;
  //   let data = {
  //     module_id: moduleId,
  ...
  //   this.subscriberService.getSubmoduleListofReadProgression(data).subscribe(res => {
  //     this.submoduleReadProgression = res.data;
  //     this.moduleWiseReadQuesCount = res.read_question;
  //     this.moduleWiseTotalQuesCount = res.module_total_question;
  //   })
  // }

  // clickProgressionPreviousPage() {
  //   this.showSubmodReadProgres = true;
  // }

  // getSubscribersQuizProgressList(data: object) {
  //   this.subscriberService
  //     .getSubscribersQuizProgress(data)
  //     .subscribe((response) => {
  //       this.quizProgressions = response.data;
  //       this.quizProgressionCount = response.totalcount;
  //     });
  // }
  getSubscribersTransactionList(data: object) {
    this.subscriberService
      .getSubscribersTransactions(data)
      .subscribe((response) => {
  ...
  //     user_id: this.route.snapshot.paramMap.get("id"),
  //   }
  //   this.subscriberService.getReadCount(data).subscribe((response) => {
  ...
  changePlan(event: any) {
    ...
    if ((currentPlanId == 7 || currentPlanId == 8 || currentPlanId == 9 || currentPlanId == 10 || currentPlanId == 11) && (this.users[0].subscriptionplanstatus == "Active" || this.users[0].subscriptionplanstatus == "trail not started yet")) {
      this.toaster.add({
        severity: "error",
        summary: "Restricted",
        detail: "You cannot select another free trial until current Free Trial has expired.",
      });
    ...
    if ((currentPlanId >= 1 && currentPlanId <= 6) || (previousPlanId >= 1 || previousPlanId <= 6)) {
      this.toaster.add({
        severity: "error",
        summary: "Restricted",
        detail: "You can't change the plan",
      });
      this.editform.get('subscription_id').setValue(previousPlanId);
    }
  ...
  changeSourceType(event: any, specialization_id: string = null) {
    ...
    if (event.value == 2) {
      ...
      this.subscriberService.getcurrentspecialization().subscribe((response) => {
        ...
        if (specialization_id) {
          this.editform.patchValue({
            current_specialization_id: specialization_id,
          });
        }
      });
      this.editform.patchValue({
    ...
    this.pageService
      .getSourceName(this.editform.value.source_type)
      .subscribe((response) => {
        this.sourceName = [{ id: null, source_name: "Select" }, ...response];
      });
  }
  getSanitizedHtml(message): SafeHtml {
    // Remove <p> tags using regex and sanitize the output
  ...
  deleteData(id: any) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.subscriberService.deleteStudenRemarks(data).subscribe((res) => {
  ...
  initializePhoneNo() {
    fetch('https://ipapi.co/json/').then(response => response.json()).then(data => {
      ...
      // this.preferredCountrywhatsapp= data.country_code.toLocaleLowerCase()
    });
  }
  ...
  getHomeCountryList() {
    this.subscriberService.getHomeCountry(2).subscribe(
      (res: any) => {
        ...
        // this.editform.get('country_id')?.reset();
      },
      (error: any) => {
        this.toaster.add({
          severity: "warning",
  ...
  onPageChangePayment(event: any) {
    ...
    const params = {
      ...
      company_id: this.editingCompanyId
    };
