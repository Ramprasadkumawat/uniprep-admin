...
export class TalentConnectService {
  ...
  importEmployer(val: any) {
    ...
    return this.httpClient.post<any>(
      ...
      catchError((err) => {
        // Fallback for environments where legacy route is still active.
        const routeNotFound =
          err?.status === 404 ||
          `${err?.error?.message || ""}`.includes("route api/import-employers could not be found");
        if (routeNotFound) {
          return this.httpClient.post<any>(
            environment.ApiUrl + "/importemployers",
            params
          );
        }
