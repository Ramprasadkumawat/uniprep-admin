...
@Component({
    selector: 'uni-learning-sub-category',
    templateUrl: './learning-sub-category.component.html',
    styleUrls: ['./learning-sub-category.component.scss'],
    standalone: false
})
export class LearningSubCategoryComponent implements OnInit {
    ...
    constructor(private fb: FormBuilder, private readingService: ReadingService,private storage: LocalStorageService,
        private toast: MessageService, private confirmationService: ConfirmationService,private router: Router,) {
        this.editImage = "";
    ...
    ngOnInit(): void {
      ...
      this.Category_id = url.split("/").pop();
    ...
    loadSubmoduleListForDropdown() {
            this.readingService.getsubmodulenames(this.Category_id).subscribe(response => {
    ...
    submitForm() {
        ...
        if (this.form.value.id == '') {
            this.readingService.addLearningSubmodule({
                ...this.form.value, image: this.selectedFile,
                 status: 1,category_id:this.Category_id
            }).subscribe((response) => {
    ...
    onClickSubmitFilter() {
        ...
        if (!formData.fromdate && !formData.todate && !formData.formatted && !formData.favourite && !formData.verified && !formData.suggestion && !formData.filter_submodule_id && !formData.pre_verified && !formData.reviewed && !formData.actioned_by && !formData.question  && !formData.category_id) {
            ...
            let req = {
                ...
                mode: 'admin',
                ...formData,
                module_category_id:this.Category_id
            };
            this.readingService.getLearningSubCategories(req).subscribe((response) => {
    ...
    editSubCategoryName(item: any) {
        ...
        this.readingService.editLearningSubCategory(item.id).subscribe(res => {
           let data = res.data;
            this.form.reset();
    ...
    updateSubCategory() {
        ...
        this.readingService.updateLearningSubCategory(req,this.form.value?.id).subscribe(data => {
            this.form.reset();
            this.getSubModuleData();
    ...
    navigateSubcategory(data: any) {
        ...
        let navigationExtras: NavigationExtras = {
            state: { ...data,moduleId: 8, country: 0, moduleName: this.subModuleName, ...filterFormData }
        }
        this.storage.set('sub-category', navigationExtras);
        this.router.navigate(['/reading/learning-hub-questions'], navigationExtras).then();
    }
}
